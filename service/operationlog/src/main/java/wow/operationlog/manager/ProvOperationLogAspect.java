...
import cn.wow.common.domain.MybatisVO;
...
import wow.operationlog.OperationLogIgnore;
import wow.operationlog.OperationLogNeeded;
...
public class ProvOperationLogAspect
{
   ...
   @Pointcut("@annotation(wow.operationlog.OperationLogNeeded)")
   ...
   {
      String error = "";
      String jsonDetail = logItem.jsonDetail(error);
   ...
   private class LogItem
   {
      ...
      String jsonDetail(String error)
      {
         ...
         {
            jsonDetail = OpLogDetailCoder.getJobDetailString(newJson, oldJson, error, jobType, operationType);
         }
         else if (operationType.onlyLogNew())
         {
            jsonDetail = OpLogDetailCoder.getJobDetailString(newJson, null, error, jobType, operationType);
         }
         else if (operationType.onlyLogOld())
         {
            jsonDetail = OpLogDetailCoder.getJobDetailString(null, oldJson, error, jobType, operationType);
         }
   ...
   private class LogItemParser
   {
      ...
      public LogItem parseBefore(JoinPoint joinPoint, OperationLogNeeded osn)
      {
         ...
         if (!findByEntity(args, logItem) && !findByEntities(args, logItem) && !findByPK(args, logItem) && !findByAnnoPK(args, osn, logItem))
         {
            return logItem;
         }
      ...
      public String getNewJson(Object[] args, LogItem logItem)
      {
         ...
         if (pos >= 0)
         {
            ...
            else if (logItem.entityFieldType == EntityFieldType.PRIMARY_KEY)
            {
              ...
              /* Object entity = entityManager.find(logItem.entityClass, pk);
               if (entity != null)
               {
                  result = JsonUtil.toJson(entity);
               }*/
       ...
       * 
       * - replace means delete then recreate, another case of update, usually used in pk-changing.
       * 
       * @param args
       * @param result
       * @return
      ...
      private boolean findByEntity(Object[] args, LogItem result)
      {
         ...
         if (pos >= 0)
         {
            ...
            if (pos < (args.length - 1))
            {
               ...
               if (target instanceof Serializable && !(target instanceof MybatisVO) && !PropertyUtil.isCollectionField(target))
               {
              ...
              if (oldpk != null)
            {
               ...
               Object oldEntity = entityManager.find(entity.getClass(), oldpk);
               if (oldEntity != null)
               {
      ...
      /**
       * create/update/delete(Collection<? extends JpaEntity> firstJpaEntityList)
       * 
       * @param args
       * @param result
       * @return
      ...
      {
         Collection<? extends Serializable> entities = null;
         for (int i = 0; i < args.length; i++)
         {
            Object o = args[i];
            if (isJpaEntityCollection(o))
            {
               entities = (Collection<? extends Serializable>)o;
         ...
         if (entities != null && !entities.isEmpty())
         {
            ...
            for (Object obj : entities)
            {
               ...
               if (id != null)
               {
                  Object oldEntity = entityManager.find(obj.getClass(), id);
                  if (oldEntity != null)
                  {
      ...
      /**
       * delete([SomeParam... aa, ] Class<? extends JpaEntity> firstJpaEntityClass, Serializable followsPk)
       * 
       * @param args
       * @param result
       * @return
      ...
      private boolean findByPK(Object[] args, LogItem result)
      {
         ...
         if (pos >= 0 && pos < (args.length - 1))
         {
            ...
            if (target instanceof Serializable && !(target instanceof Serializable) && !PropertyUtil.isCollectionField(target))
            {
            ...
            {
               Object oldEntity = entityManager.find(result.entityClass, id);
               if (oldEntity != null)
               {
                  result.oldJson = JsonUtil.toJson(oldEntity);
               }
